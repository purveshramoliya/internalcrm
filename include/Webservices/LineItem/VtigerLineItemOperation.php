...
class VtigerLineItemOperation extends VtigerActorOperation {
	...
	public function calculateNetprice($element) {
		...
		$productId = $element['parent_id'];
		 global $adb;
                $recId = $element['productid'];
                $idsOfCreated = explode('x', $recId);
                $idsOfCreated = $idsOfCreated[1];
                $element['productname'] = $this->getLable($idsOfCreated); 
	...
	public function getLable($id) {
            global $adb,$log;
            $query = "SELECT * FROM `vtiger_crmentity` where crmid = ? and deleted = ? ";
