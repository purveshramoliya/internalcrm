...
{strip}
    <div class="modal-dialog">
        <div id="convertLeadContainer" class='modelContainer modal-content'>
            ...
            {else}
                {assign var=HEADER_TITLE value={vtranslate('LBL_CONVERT_USER', $MODULE)}|cat:" "|cat:{$RECORD->getName()}}
                {include file="ModalHeader.tpl"|vtemplate_path:$MODULE TITLE=$HEADER_TITLE}
                <form class="form-horizontal" id="convertLeadForm" method="post" action="registerajax.php">
                                            ...
                                            <div data-parent="#leadAccordion" data-toggle="collapse" class="accordion-toggle moduleSelection" href="#{$MODULE_NAME}_FieldInfo">
                                                ...
                                                <h5>
                                                    <input id="{$MODULE_NAME}Module" class="convertLeadModuleSelection" data-module="{vtranslate($MODULE_NAME,$MODULE_NAME)}" value="{$MODULE_NAME}" type="checkbox" 
                                                           ...
                                                           &nbsp;&nbsp;&nbsp;{vtranslate('LBL_CREATE', $MODULE)}&nbsp;User
                                                    </h5>
                                                </div>
                                            </div>
                                            <div id="{$MODULE_NAME}_FieldInfo" class="{$MODULE_NAME}_FieldInfo accordion-body collapse fieldInfo {if $CONVERT_LEAD_FIELDS['Accounts'] && $MODULE_NAME == "Accounts"} in {/if}">
                                                <hr>
                                                {foreach item=FIELD_MODEL from=$MODULE_FIELD_MODEL}
                                                    <div class="row">
                                                    {if $MODULE_NAME eq 'Accounts' and $FIELD_MODEL->get('label') neq 'Password'  and $FIELD_MODEL->get('label') neq 'Confirm Password'}
