...
// 		}
		...
		if($email == 'buyleads@indiamart.com'){
		    $starter = strpos(trim($description),"Contact Details:") + 16;
            $ender =  strpos(trim($description),"Click to call");
            $mainName = substr(trim($description),$starter,$ender-$starter);
		}
		...
		if($email == 'buyleads@indiamart.com'){
		    $lead->column_fields['firstname'] = $mainName;
		    	$lead->column_fields['lastname'] =  '';
		} else {
		...
		preg_match_all("/\(?([0-9]{3})\s*\)?\s*-?\s*([0-9]{3})\s*-?\s*([0-9]{4})/",$description,$matches);
        $phoneNumbers = array_unique($matches[0]);
		$lead->column_fields['phone'] = $phoneNumbers[0];
		
		preg_match_all("/[\._a-zA-Z0-9-]+@[\._a-zA-Z0-9-]+/i",$description,$emailMatches);
		$emailWithProperName = '';
        $emails = array_unique($emailMatches[0]);
        if(strpos($emails[0] , ".com")){
            $emailArray = explode(".com",$emails[0]);
            if($emailArray[1] == ""){
                $emailWithProperName = $emails[0];
            } else {
                $emailWithProperName = $emailArray[0] . '.com';
                $lead->column_fields['city'] =  $emailArray[1];
            }
        } else {
            $emailWithProperName = $emails[0];
        }
        
		$lead->column_fields['email'] = $emailWithProperName;
