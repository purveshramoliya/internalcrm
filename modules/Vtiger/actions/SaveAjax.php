...
class Vtiger_SaveAjax_Action extends Vtiger_Save_Action {
	...
	public function process(Vtiger_Request $request) {
		...
		try {
			...
			if($request->get('module') == 'Leads' && (($request->get('field') == 'cf_952' && ($request->get('value') == 'Lost Lead' || $request->get('value') == 'Junk Lead' )))){
				$recordModel = Vtiger_Record_Model::getInstanceById($request->get('record'));
				$leadStatus = $recordModel->get('qcf_1049');
				if(empty($leadStatus)){
					$response->setError("Please First update Closed Lost or Junk Reason Information");
				} else {
