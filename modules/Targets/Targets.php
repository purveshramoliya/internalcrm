...
class Targets extends Vtiger_CRMEntity {
	var $table_name = 'vtiger_targets';
	var $table_index= 'targetsid';

	/**
	 * Mandatory table for supporting custom fields.
	 */
	var $customFieldTable = Array('vtiger_targetscf', 'targetsid');

	/**
	 * Mandatory for Saving, Include tables related to this module.
	 */
	var $tab_name = Array('vtiger_crmentity', 'vtiger_targets', 'vtiger_targetscf');
	...
	var $related_tables = Array( 
					'vtiger_targetscf' => Array('targetsid')
					);
	...
	var $list_fields = Array (
		...
		'Targets No' => Array('targets', 'targetsno'),
/*'Targets No'=> Array('targets', 'targetsno'),*/
		'Assigned To' => Array('crmentity','smownerid')
	);
	var $list_fields_name = Array (
		/* Format: Field Label => fieldname */
		'Targets No' => 'targetsno',
/*'Targets No'=> 'targetsno',*/
		'Assigned To' => 'assigned_user_id'
	);

	// Make the field link to detail view
	var $list_link_field = 'targetsno';
...
/*'Targets No'=> 'targetsno',*/
	...
	var $popup_fields = Array ('targetsno');

	// For Alphabetical search
	var $def_basicsearch_col = 'targetsno';

	// Column value to use on detail view record text display
	var $def_detailview_recname = 'targetsno';

	// Used when enabling/disabling the mandatory fields for the module.
	// Refers to vtiger_field.fieldname values.
	var $mandatory_fields = Array('targetsno','assigned_user_id');

	var $default_order_by = 'targetsno';
	...
	*/
	function vtlib_handler($moduleName, $eventType) {
		global $adb;
 		if($eventType == 'module.postinstall') {
			...
			Targets::checkWebServiceEntry();
			Targets::createUserFieldTable($moduleName);
			Targets::addInTabMenu($moduleName);
	...
	static function checkWebServiceEntry() {
		...
		$sql       =  "SELECT count(id) AS cnt FROM vtiger_ws_entity WHERE name = 'Targets'";
		...
		if($adb->num_rows($result) > 0)
		{
			...
			if($no == 0)
			{
				...
				$ws_entitySql = "INSERT INTO vtiger_ws_entity ( id, name, handler_path, handler_class, ismodule ) VALUES".
						  " (?, 'Targets','include/Webservices/VtigerModuleOperation.php', 'VtigerModuleOperation' , 1)";
