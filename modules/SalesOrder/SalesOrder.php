...
class SalesOrder extends CRMEntity {
	...
	function save_module($module)
	{
		...
		$PtQuery = $this->db->pquery("SELECT * from vtiger_salesorder where salesorderid = " . $this->id);
		$PtFieldsCount =$this->db->num_rows($PtQuery);
		$currencyId = '';
		for ($i = 0; $i < $PtFieldsCount; $i++) {
			$currencyId = $this->db->query_result($PtQuery, $i, 'currency_id');
			$conversion_rate =(int) $this->db->query_result($PtQuery, $i, 'conversion_rate');
		}

		$PtQuery = $this->db->pquery("SELECT * from vtiger_currency_info where id = " . $currencyId);
		$PtFieldsCount =$this->db->num_rows($PtQuery);
		$conversion_rate = '';
		for ($i = 0; $i < $PtFieldsCount; $i++) {
		...
		$PtQuery = $this->db->pquery("SELECT * from vtiger_inventoryproductrel where id=" . $this->id);
		$PtFieldsCount =$this->db->num_rows($PtQuery);
		for ($i = 0; $i < $PtFieldsCount; $i++) {
			
			$Pt_Productname = $this->db->query_result($PtQuery, $i, 'productname');
			$productId = (int) $this->db->query_result($PtQuery, $i, 'productid');
			$quantity = (int)$this->db->query_result($PtQuery, $i, 'quantity');
			$listprice =(double) $this->db->query_result($PtQuery, $i, 'listprice');

			$taskTypeInstance = Vtiger_Record_Model::getInstanceById($productId);
			$focus = CRMEntity::getInstance('Assets');
			$focus->column_fields['product'] = $productId;
		
			$unitPrice = (double) $taskTypeInstance->get("unit_price");
			$category = $taskTypeInstance->get('productcategory');

			if((int)$currencyId == 1){
				$focus->column_fields['cf_918'] = ($quantity * $listprice * $conversion_rate) - ($quantity * $unitPrice * $conversion_rate);
			} else {
				$focus->column_fields['cf_918'] = $quantity * $listprice - $quantity * $unitPrice;
			}

			$focus->column_fields['assigned_user_id'] = $this->column_fields['assigned_user_id'];
			$focus->column_fields['assetname'] = $category;
			$focus->save("Assets");
		}
	}
