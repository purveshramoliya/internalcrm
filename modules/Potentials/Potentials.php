...
class Potentials extends CRMEntity {
	...
	function getPreferedCurrencyConversionRateofUser($userId) {
		$PtQuery = $this->db->pquery("SELECT * from vtiger_users where id = " . $userId);
		$PtFieldsCount =$this->db->num_rows($PtQuery);
		$currecyId = '';
		for ($i = 0; $i < $PtFieldsCount; $i++) {
			$currecyId = $this->db->query_result($PtQuery, $i, 'currency_id');
		}

		$PtQuery = $this->db->pquery("SELECT * from vtiger_currency_info where id = " . $currecyId);
		$PtFieldsCount =$this->db->num_rows($PtQuery);
		$conversion_rate = '';
		for ($i = 0; $i < $PtFieldsCount; $i++) {
			$conversion_rate = $this->db->query_result($PtQuery, $i, 'conversion_rate');
		}
...
// 	    $recordInstance = Vtiger_Record_Model::getInstanceById($this->id);
...
// 		$query = "SELECT vtiger_projectmilestone.*,vtiger_projectmilestonecf.* from vtiger_projectmilestone
// 		left join vtiger_crmentity on (vtiger_projectmilestone.projectmilestoneid = vtiger_crmentity.crmid)
// 		left join vtiger_projectmilestonecf on (vtiger_projectmilestone.projectmilestoneid = vtiger_projectmilestonecf.projectmilestoneid)
// 		where vtiger_crmentity.smownerid =  $user and vtiger_crmentity.deleted = 0 
// 		and vtiger_projectmilestonecf.cf_914 = '$startDate' and vtiger_projectmilestonecf.cf_916 = '$endDate' ";
...
// 			while ($rowCount > 0) {
// 				$rowData = $this->db->query_result_rowdata($result,$rowCount-1);
...
// 		$query = "SELECT * FROM `vtiger_potential` 
...
// 			while ($rowCount > 0) {
// 				$rowData = $this->db->query_result_rowdata($result,$rowCount-1);
// 				$achieved = $achieved + $rowData['amount'];
// 				--$rowCount;
// 			}
// 		}
...
// 		$curencyConversionValue = $this->getPreferedCurrencyConversionRateofUser($user);

// 		foreach($recentlyClosedProjects as $userQuotaentry){
// 			$gap = ($userQuotaentry['cf_908'] * $curencyConversionValue ) - ($achieved * $curencyConversionValue );
// 			$gap = $gap / $curencyConversionValue;
// 			$this->db->pquery("update vtiger_projectmilestonecf set cf_924 = $totaldealAmount, cf_922 = $conversionRatio, cf_926 = $numberOFDealsClosed, cf_910 = $achieved, cf_928 = $numberOFDeals , cf_912 = $gap where projectmilestoneid = " . $userQuotaentry['projectmilestoneid']);
