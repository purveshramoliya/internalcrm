...
class Contacts_ConvertFAQ_Action extends Vtiger_Action_Controller {
	...
	$body='<html>
        ...
        include("ExDocuments/mpdf/mpdf.php");

        $mpdf=new mPDF('c','A4','','',15,15,15,15,15,15);  
        $stylesheet = file_get_contents('ExDocuments/mpdfstyletables.css');
        $mpdf->WriteHTML($stylesheet,1);
        //write html to PDF
        $m=$mpdf->WriteHTML($body,2);
        //output pdf
        $xfile=$mpdf->Output('Offerlatter-'.$employeename.'-'.$month.'-'.$year.'.pdf','D');
