...
function saveInventoryProductDetails(&$focus, $module, $update_prod_stock='false', $updateDemand='')
{
	...
	for($i=1; $i<=$tot_no_prod; $i++)
	{
		...
		$query = 'INSERT INTO vtiger_inventoryproductrel(id, productid, productname, sequence_no, quantity, listprice, comment, description, purchase_cost, margin)
					VALUES(?,?,?,?,?,?,?,?,?,?)';
		$qparams = array($focus->id,$prod_id,$prod_name,$prod_seq,$qty,$listprice,$comment,$description, $purchaseCost, $margin);
		$adb->pquery($query,$qparams);
