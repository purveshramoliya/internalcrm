    ...
    public function saveRecord(Vtiger_Record_Model $recordModel) {
        ...
        $recordId = $templateid = $recordModel->getId();
         if (empty($templateid)) {
            ...
            $result = $db->pquery($sql, array($recordModel->get('templatename')));
            $numberOfRows = $db->num_rows($result);
            
            if($numberOfRows > 0){
                die("Email Template Name Already Exits, please choose diffrent Email Template Name <a class='cancelLink' href='javascript:history.back()'>Go Back </a>");
            }
         }
         
        $createdUserName = $current_user->first_name .' '.$current_user->last_name;
        ...
        if (empty($templateid)) {
            ...
            $sql = "INSERT INTO vtiger_emailtemplates(templatename, subject, description, module, body, deleted, systemtemplate, templateid,templatepath) VALUES (?,?,?,?,?,?,?,?,?)";
        } else {
            if ($systemtemplate) {
        ...
        if (!empty($recordId) && $systemtemplate) {
            ...
            $params = array(decode_html($recordModel->get('templatename')), decode_html($recordModel->get('subject')),
                ...
                $systemtemplate, $templateid,$createdUserName);
        }
